...
const slides = images.map((i) => ({ src: i.src, width: i.width, height: i.height, alt: i.caption, description: i.caption }))
  ...
  return (
    <>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {screens.map((s, i) => (
          <figure key={s.src}>
            <button type="button" onClick={() => setIndex(i + 1)} className={thumbClass} aria-label={`${s.caption} 크게 보기`}>
              <Image
                src={s.src}
                alt={s.caption}
                width={s.width}
