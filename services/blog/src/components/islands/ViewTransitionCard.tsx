// app/[locale]/(ko-only)/dev/view-transition/card/page.tsx 이식. client:only="react" (ViewTransitionDemo.tsx와
// 동일 이유). next/image(fill) → <img class="absolute inset-0 w-full h-full object-cover ...">(fill 시맨틱
// 수동 재현, 부모에 이미 relative가 있어 동일하게 꽉 채움). next/link → plain <a>.
// biome-ignore-all lint/performance/noImgElement: Astro 정적 사이트엔 next/image 대응물이 없다(LumosGallery.tsx 선례와 동일)
import { cx, PLACES, ViewTransitionSafe } from './viewTransitionUtils'

function DynamicBackground({ className }: { className?: string }) {
  return (
    <div
      className={cx(
        'absolute w-96 h-96 bg-blue-500 opacity-50 blur-3xl top-20 left-1/4 transform rotate-12 skew-y-6 rounded-full',
        className,
      )}
    />
  )
}

function LeftSideMenu() {
  return (
    <div className="relative w-full overflow-hidden m-h-screen md:w-1/2 bg-emerald-200">
      <div className="top-0 bottom-0 left-0 flex flex-col p-8 md:absolute">
        <DynamicBackground />
        <div className="z-10 my-4">
          <ViewTransitionSafe name="experimental-label">
            <b className="inline-block text-gray-700">{'<ViewTransition>'}</b>
          </ViewTransitionSafe>
          <div className="flex items-center justify-between mb-4">
            <a href="/" className="text-blue-gray-500 hover:underline">
              ← Back
            </a>
          </div>
        </div>

        <div className="z-10 flex items-center gap-2 mb-4 text-gray-800">
          <ViewTransitionSafe name="sticker-icon">
            <a href="/card/overview">
              <span className="flex items-center justify-center w-8 h-8 transition-all duration-150 bg-gray-800 rounded-full hover:scale-120" />
            </a>
          </ViewTransitionSafe>
          <span className="font-medium">Las Ciudades</span>
        </div>

        <div className="z-10 flex flex-col justify-center flex-1">
          <h1 className="space-y-2 font-serif text-6xl">
            <div>
              Explore <span className="block font-serif text-4xl md:inline-block">The cities.</span>
            </div>
          </h1>
        </div>
      </div>
    </div>
  )
}

export default function ViewTransitionCard() {
  return (
    <div className="flex flex-col min-h-screen md:flex-row bg-red-50 ">
      <ViewTransitionSafe name="left-side-bar">
        <LeftSideMenu />
      </ViewTransitionSafe>

      <ViewTransitionSafe name="right-side-bar">
        <div className="w-full p-2 md:w-1/2 md:p-8">
          <h2 className="px-2 mt-4 text-xl font-medium text-gray-700">Spots</h2>
          <div className="flex flex-wrap gap-8 p-2 space-y-4">
            {PLACES.map((place) => (
              <a
                key={place.id}
                href={`/card/${place.slug}`}
                className="hover:bg-gray-50 transition-colors w-full md:w-[300px] h-[300px] items-stretch"
              >
                <div className="relative flex-shrink-0 w-full h-full rounded-lg overflow-clip group">
                  <ViewTransitionSafe name={`place-image-${place.slug}`}>
                    <img
                      loading="eager"
                      decoding="sync"
                      src={place.image || '/placeholder.svg'}
                      alt={place.name}
                      className="object-cover absolute inset-0 w-full h-full rounded-lg transition-transform overflow-clip group-hover:scale-110"
                    />
                  </ViewTransitionSafe>
                  <ViewTransitionSafe name={`place-name-${place.slug}`}>
                    <div className="absolute right-4 bottom-4 text-3xl text-gray-100 bg-opacity-50 rounded-xl filter [text-shadow:0px_0px_8px_#111]">
                      {place.name}
                    </div>
                  </ViewTransitionSafe>
                </div>
              </a>
            ))}
          </div>
        </div>
      </ViewTransitionSafe>
    </div>
  )
}
