// app/[locale]/(ko-only)/talks/[slug]/PDFPresentation.tsx 이식 (verbatim) — react-pdf는 브라우저 전용 API(canvas 등)에
// 의존해 SSR 불가라(원본도 dynamic ssr:false) client:only="react"로 마운트한다. unpkg worker/cMap/font URL 그대로 유지
// (CSP에 unpkg 허용됨).
...
export default function PDFPresentation({ pdfUrl, title }: PDFPresentationProps) {
  ...
  const calculatePdfDimensions = useCallback(() => {
    ...
    const headerHeight = isFullscreen ? 0 : 56
    const footerHeight = isFullscreen ? 0 : 100
            ...
            loading={
              <div className="flex justify-center items-center w-full" style={{ height: pdfHeight }}>
                <div className="text-center">
                  <div className="inline-block w-12 h-12 rounded-full border-4 border-gray-200 animate-spin border-t-primary-500" />
          ...
          <div className="flex gap-3 items-center">
            ...
            <div className="hidden gap-2 md:flex">
              {(() => {
                ...
                const endPage = Math.min(numPages, startPage + maxDots - 1)
