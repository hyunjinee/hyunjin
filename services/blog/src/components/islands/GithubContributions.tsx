// app/[locale]/(ko-only)/kakaoent/GithubContributions.tsx 이식 — 호버 툴팁 상태가 있어 island로 유지.
// next/link Link → 일반 <a> (내부 네비게이션 없이 외부 github.com 링크 하나뿐).
  ...
  return (
    <div>
      <p className="mb-3 text-sm text-gray-600 dark:text-gray-400">
        <span className="font-semibold text-gray-900 dark:text-gray-100">{year}</span> · {total.toLocaleString()}{' '}
        contributions
      </p>
      <div className="overflow-x-auto" onScroll={() => onTip(null)}>
        {/* biome-ignore lint/a11y/noStaticElementInteractions: 원본(app/[locale]/(ko-only)/kakaoent/GithubContributions.tsx)과 동일한 호버 툴팁 전용 위젯 — 포인터 전용 인터랙션은 알려진 기존 한계, 그대로 이식 */}
        {/* biome-ignore lint/a11y/useKeyWithMouseEvents: 위와 동일 — 키보드 접근성은 원본에도 없던 기능, 이 포트에서 새로 만들지 않음 */}
        <div
        ...
        >
          {Array.from({ length: padding }, (_, i) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: 패딩용 빈 span — 데이터 없는 고정 개수 필러라 순서·상태가 없어 index가 곧 안정적인 키
