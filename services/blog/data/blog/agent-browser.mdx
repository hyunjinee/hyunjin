...
## 기존 브라우저 자동화 도구는 AI를 위해 만들어지지 않았다

Playwright, Selenium, Puppeteer등 브라우저를 코드로 제어하는 자동화 도구는 다양하다.
...
결국 핵심 문제는 **기존 도구가 사람이 DOM을 이해한 상태에서 쓰도록 설계되었고, AI 에이전트가 웹을 "인식"하는 방식에 맞춰져 있지 않다**는 것이다.

**[agent-browser](https://github.com/vercel-labs/agent-browser)**는 이 문제를 처음부터 다시 설계해서 해결한다.
접근성 트리 기반 스냅샷, 시맨틱 Ref 시스템, Rust 네이티브 바이너리 — 각각이 위의 문제 하나씩에 대응한다.
