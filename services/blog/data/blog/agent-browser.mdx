...
| 실행 모델 | 셸 커맨드 | MCP 서버 | 라이브러리 API | 라이브러리 API |
...
Playwright MCP는 agent-browser와 가장 직접적으로 비교되는 도구다. 둘 다 접근성 트리와 Ref 시스템을 사용하지만, 세부 접근이 다르다.

**1. 스냅샷 필터링** — Playwright MCP는 접근성 트리를 비교적 그대로 반환한다. `generic` 래퍼 노드에도 ref를 할당하고, 텍스트를 중복 포함한다. agent-browser는 인터랙티브 요소만 남기는 공격적 필터링으로 같은 페이지를 82% 적은 토큰으로 표현한다.

**2. 클릭 응답** — Playwright MCP는 클릭 후 전체 스냅샷을 자동으로 다시 반환한다. 에이전트 루프가 단순해지지만, 10단계 워크플로우에서 수십만 토큰의 차이로 이어진다. agent-browser는 `✓ Done` 2 토큰만 반환하고, 에이전트가 필요할 때만 `snapshot`을 호출한다.

**3. 실행 모델** — Playwright MCP는 MCP 서버로 작동하며 21~45+개의 도구를 노출한다. agent-browser는 하나의 셸 커맨드로, MCP를 지원하지 않는 환경에서도 바로 작동한다.

**4. 보안** — Playwright MCP에도 `--allowed-hosts`와 `--secrets`가 있지만, 공식 문서에서 "보안 경계가 아니다"라고 명시한다. agent-browser는 도메인 필터(서브리소스/WebSocket 포함), 액션 정책, AES-256-GCM 인증 볼트, CSPRNG 콘텐츠 경계를 내장한다.

**5. Ref의 차이** — 둘 다 ref를 사용하지만, agent-browser의 ref는 stale 시 시맨틱 속성(role + name)으로 재탐색하는 fallback 경로가 있다. Playwright MCP의 ref는 스냅샷 시점에만 유효하며, 별도의 fallback은 없다.
