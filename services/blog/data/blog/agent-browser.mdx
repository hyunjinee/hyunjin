...
Playwright, Selenium, Puppeteer—브라우저를 코드로 제어하는 도구는 다양하다.
...
AI 에이전트가 `click @e3`을 실행할 때마다 새 프로세스가 뜨지만, 실제 브라우저 연결과 세션 상태는 백그라운드에서 계속 살아있는 데몬이 유지한다.
CLI 프로세스는 데몬에 명령을 전달하고 결과를 받아서 출력만 한다.ㅍ
