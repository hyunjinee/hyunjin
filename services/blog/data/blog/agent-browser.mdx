...
AI 에이전트에 연결하면 이 전제가 무너지면서 아래 문제들이 드러난다.

**토큰으로 변환된 DOM은 비싸고 비효율적이다.**
AI 에이전트에게 필요한 건 페이지의 모든 `<div>`와 CSS 클래스가 아니라, "로그인 버튼이 어디 있고 검색창에 뭘 입력할 수 있는지"다.
일반적인 웹 페이지의 DOM 표현은 수천에서 수만 토큰을 차지하고, 복잡한 SPA라면 수십만 토큰을 넘길 수 있다.
페이지 하나를 "보는" 데만 이 비용을 쓰는 건, 수십 번의 페이지 탐색이 필요한 실제 작업에서 치명적이다.

**CSS 셀렉터는 AI에게 너무 깨지기 쉽다.**
AI가 생성한 `#app > div:nth-child(3) > ul > li:first-child > a` 같은 셀렉터는 DOM 구조가 조금만 바뀌어도 무너진다.
사람은 시각적으로 "검색 버튼"을 인식하지만, AI는 매번 정확한 셀렉터 문자열을 생성해야 한다.

**도구가 많으면 모델의 의사결정이 어려워진다.**
Playwright MCP는 26개 이상의 도구를 노출한다. 모델은 매 턴마다 이 중 어떤 도구를 어떤 파라미터로 호출할지 결정해야 한다.
선택지가 많아질수록 잘못된 도구를 고르거나 파라미터를 혼동할 확률이 올라간다.

결국 핵심 문제는 하나다: **기존 도구는 사람이 DOM을 이해한 상태에서 쓰도록 설계되었고, AI 에이전트가 웹을 "인식"하는 방식에 맞춰져 있지 않다.**

**[agent-browser](https://github.com/vercel-labs/agent-browser)**는 이 문제를 처음부터 다시 설계해서 해결한다.
접근성 트리 기반 스냅샷, 시맨틱 Ref 시스템, Rust 네이티브 바이너리 — 각각이 위의 문제 하나씩에 대응한다. 하나씩 살펴보자.

---

## 접근성 트리: AI가 웹을 읽는 방법

agent-browser의 핵심 아이디어는 단순하다. DOM 대신 **접근성 트리(Accessibility Tree, AX Tree)**를 AI에게 보여주는 것이다.
...
이 접근성 트리를 어떻게 가져올까? agent-browser는 CDP(Chrome DevTools Protocol)를 사용한다.
...
이론만으로는 체감이 어렵다. naver.com의 검색 영역을 HTML로 넘기면 이런 모습이다:
...
특히 클릭 응답의 차이가 결정적이다. 
Playwright MCP는 클릭할 때마다 전체 스냅샷을 다시 반환하지만, agent-browser는 `✓ Done` 2 토큰만 돌려준다.
10단계 워크플로우라면 이 차이가 **수십만 토큰**으로 벌어진다.
...
`-C` 플래그를 추가하면 `cursor: pointer` CSS 속성이나 `onclick`, `tabindex` 속성을 가진 요소도 인터랙티브로 분류한다.
내부적으로 `Runtime.evaluate`를 통해 DOM에서 이런 요소를 탐색하고, AX 트리의 `backend_node_id`와 매칭해서 ref를 부여한다.

### 크로스 오리진 iframe

단일 페이지 앱이 아닌 실제 웹에서는 결제 위젯, 광고, 임베디드 콘텐츠 등 **크로스 오리진 iframe**이 빈번하다.
크로스 오리진 iframe은 보안 정책상 부모 페이지에서 직접 접근할 수 없기 때문에, 일반적인 접근성 트리 조회로는 내부 요소가 보이지 않는다.
...
naver.com에서 `agent-browser click @e99`를 실행하면, CLI 프로세스가 상주 데몬에 명령을 전달하고 내부에서 이런 일이 일어난다:
...
agent-browser는 3-tier 구조다. Rust 네이티브 바이너리가 CDP로 Chromium과 직접 통신하되, 그 사이에 **상주 데몬**이 위치한다.
...
셸에서 `agent-browser click @e99`를 실행하면 가벼운 CLI 프로세스가 생성된다.
이 프로세스는 Unix Socket(macOS/Linux) 또는 TCP(Windows)로 상주 데몬에 명령을 전달하고, 결과를 받아서 출력만 한다.
...
데몬 안의 `CdpClient`는 `tokio-tungstenite`로 Chromium과 WebSocket 연결을 유지한다.
...
지금까지 살펴본 설계가 기존 도구와 어떤 차이를 만드는지, 가장 자주 비교되는 Playwright MCP를 중심으로 정리해보자.
...
**1. 토큰 효율** — Playwright MCP는 26개 이상의 도구를 노출해 모델의 선택 복잡도를 높이고, 클릭할 때마다 전체 스냅샷을 다시 반환한다. agent-browser는 셸 명령 하나로 동작하므로 도구 선택 부담이 없고, 클릭 응답은 `✓ Done` 2 토큰이 전부다.

**2. 요소 참조** — Playwright MCP에서 AI는 `#search-form > input[type="text"]` 같은 CSS 셀렉터를 생성해야 한다. agent-browser는 `@e5`로 충분하다. ref는 시맨틱 속성(role + name)으로 요소를 식별하므로 DOM 구조 변경에 영향받지 않는다.

**3. 실행 모델** — Playwright MCP는 MCP 서버로 작동하여 AI에게 Playwright API를 노출한다. agent-browser는 Rust 네이티브 바이너리가 CDP로 직접 통신한다. 셸 명령을 실행할 수 있는 모든 AI 에이전트(Claude Code, Cursor, GitHub Copilot, Codex 등)에서 바로 사용할 수 있으며, MCP를 지원하지 않는 환경에서도 작동한다.

**4. 보안** — Playwright MCP에는 AI 에이전트 전용 보안 기능이 없다. agent-browser는 도메인 허용목록, 액션 정책, 인증 볼트, 콘텐츠 경계, 출력 제한 등 다층 보안을 내장하고 있다.

**5. 상태 관리** — 세션 암호화, 프로필 영속성, 멀티 세션 격리, 크로스 도메인 상태 저장 — agent-browser는 이 모든 것을 도구 자체에 내장하고 있다.
