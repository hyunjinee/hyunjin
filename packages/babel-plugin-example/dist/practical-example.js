...
exports.default = practicalBabelPlugin;
const utils_1 = require("./utils");
function practicalBabelPlugin(babel) {
    const { types: t, template } = babel;
    const nodeBuilder = new utils_1.NodeBuilder(t);
    return {
        ...
        visitor: {
            Program(path, state) {
                ...
                // 파일이 처리 대상인지 확인
                if (!(0, utils_1.shouldProcessFile)(state.filename, opts)) {
                    return;
                }
          ...
          `)();
                    path.node.body.unshift(perfHelper);
                }
            },
            // 함수에 성능 모니터링 추가
            FunctionDeclaration(path, state) {
                ...
                const wrappedFunction = t.variableDeclaration('const', [
                    t.variableDeclarator(t.identifier(functionName), t.callExpression(t.identifier('__measurePerformance'), [
        ...
        `);
            ...
            JSXElement(path, state) {
                ...
                if (opts.wrapWithTryCatch?.includes('ReactComponents')) {
                    const errorBoundary = t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('ErrorBoundary'), [
                        t.jsxAttribute(t.jsxIdentifier('fallback'), t.jsxExpressionContainer(t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('div'), []), t.jsxClosingElement(t.jsxIdentifier('div')), [t.jsxText('Something went wrong')]))),
                    ], false), t.jsxClosingElement(t.jsxIdentifier('ErrorBoundary')), [path.node], false);
...
*/
//# sourceMappingURL=practical-example.js.map