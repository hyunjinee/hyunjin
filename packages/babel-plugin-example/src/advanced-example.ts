...
const pluginStateMap = new WeakMap<PluginPass, Set<string>>()

export default function advancedBabelPlugin(babel: typeof import('@babel/core')): PluginObj<PluginPass> {
  ...
  return {
    ...
    pre() {
      ...
      pluginStateMap.set(this, new Set<string>())
    },
    ...
    visitor: {
      ...
      ArrowFunctionExpression(path: NodePath<t.ArrowFunctionExpression>, state) {
        const opts = state.opts as AdvancedPluginOptions
        ...
        if (opts.transformArrowFunctions) {
          ...
          path.replaceWith(
            t.functionExpression(null, path.node.params, body, path.node.generator ?? false, path.node.async ?? false),
          )
        }
      },
